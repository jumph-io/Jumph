...
namespace Jumph\Bundle\EmailBundle\Filter\Subscriber;
...
class FilterSubscriber implements EventSubscriberInterface
{
    ...
    public static function getSubscribedEvents()
    {
        return array(
            ...
            'lexik_form_filter.apply.orm.email_filter_name' => array('filterName'),

            // if a Doctrine\DBAL\Query\QueryBuilder is passed to the lexik_form_filter.query_builder_updater service
            'lexik_form_filter.apply.dbal.email_filter_name' => array('filterName'),
        );
    }
    ...
    public function filterName(ApplyFilterEvent $event)
    {
        ...
        if (!empty($values['value'])) {
            $qb->andWhere(
                $expr->orX()
                    ->add($expr->like('e.subject', "'%".$values['value']."%'"))
            );
        }
