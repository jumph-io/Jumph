...
namespace Jumph\Bundle\UserBundle\Filter\Subscriber;
...
class FilterSubscriber implements EventSubscriberInterface
{
    ...
    public function filterName(ApplyFilterEvent $event)
    {
        ...
        if (!empty($values['value'])) {
            $qb->andWhere(
                $expr->orX()
                    ->add($expr->like('u.username', "'%".$values['value']."%'"))
                    ->add($expr->like('u.firstname', "'%".$values['value']."%'"))
                    ->add($expr->like('u.lastname', "'%".$values['value']."%'"))
            );
        }
