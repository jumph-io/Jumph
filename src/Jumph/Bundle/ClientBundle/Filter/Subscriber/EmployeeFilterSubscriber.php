...
class EmployeeFilterSubscriber implements EventSubscriberInterface
{
    /**
     * Events to listen to.
     *
    ...
    public static function getSubscribedEvents()
    {
        return array(
            ...
            'lexik_form_filter.apply.orm.employee_filter_name' => array('filterName'),

            // if a Doctrine\DBAL\Query\QueryBuilder is passed to the lexik_form_filter.query_builder_updater service
            'lexik_form_filter.apply.dbal.employee_filter_name' => array('filterName'),
        );
    }
    ...
    public function filterName(ApplyFilterEvent $event)
    {
        ...
        if (!empty($values['value'])) {
            $qb->andWhere(
                $expr->orX()
                    ->add($expr->like('e.firstname', "'%".$values['value']."%'"))
                    ->add($expr->like('e.lastname', "'%".$values['value']."%'"))
            );
        }
